...
object LlmChatModelHelper {
  ...
  fun runInference(
    ...
    val instance = model.instance as? LlmModelInstance
    if (instance == null) {
      Log.e(TAG, "Model instance is null, cannot run inference for model: ${model.name}")
      resultListener("❌ AI model not initialized. Please try again after model initialization is complete.", true)
      return
    }
    ...
    try {
      ...
      Log.e(TAG, "MediaPipe native crash during inference for model: ${model.name}", e)
      resultListener("❌ AI inference failed due to native library error. The model may be corrupted or incompatible with your device. Please try restarting the app or redownloading the model.", true)
      
      // Attempt to reset the session to recover from the crash
      try {
        resetSession(model)
        Log.i(TAG, "Session reset after native crash for model: ${model.name}")
      } catch (resetException: Exception) {
        Log.e(TAG, "Failed to reset session after native crash", resetException)
        // Mark model instance as null to force reinitialization
        model.instance = null
      }
    }
  }
