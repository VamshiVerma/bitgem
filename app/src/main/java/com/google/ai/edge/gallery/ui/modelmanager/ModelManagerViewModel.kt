...
open class ModelManagerViewModel(
  ...
  init {
    loadModelAllowlist()
    // Auto-initialize available models for BitGem
    autoInitializeModelsForBitGem()
  }
  
  private fun autoInitializeModelsForBitGem() {
    viewModelScope.launch(Dispatchers.IO) {
      // Wait for model allowlist to load
      delay(3000)
      
      // Find any downloaded models and auto-initialize them
      val chatModels = com.google.ai.edge.gallery.data.TASK_LLM_CHAT.models
      if (chatModels.isNotEmpty()) {
        Log.d(TAG, "BitGem: Checking ${chatModels.size} models for auto-initialization")
        
        chatModels.forEach { model ->
          if (isModelDownloaded(model) && model.instance == null) {
            Log.d(TAG, "BitGem: Auto-initializing model: ${model.name}")
            
            // Use the stored context from constructor
            initializeModel(
              context = context,
              task = com.google.ai.edge.gallery.data.TASK_LLM_CHAT, 
              model = model,
              force = false
            )
          } else if (!isModelDownloaded(model)) {
            Log.d(TAG, "BitGem: Model ${model.name} not downloaded, skipping auto-init")
          } else if (model.instance != null) {
            Log.d(TAG, "BitGem: Model ${model.name} already initialized")
          }
        }
      } else {
        Log.d(TAG, "BitGem: No chat models available for auto-initialization")
      }
    }
  }
