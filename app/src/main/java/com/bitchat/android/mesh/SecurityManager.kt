...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    fun validatePacket(packet: BitchatPacket, peerID: String): Boolean {
        ...
        // Add small tolerance for timing variations to prevent false positives  
        // This handles cases where packet creation and validation happen in quick succession
        val TIMING_TOLERANCE = 10L // 10ms tolerance for small clock differences
        ...
        // Log timing details for debugging when there are small differences
        if (timeDiff > 0 && timeDiff <= TIMING_TOLERANCE) {
            Log.v(TAG, "Small timing difference accepted: current=$currentTime, packet=$packetTime, diff=${timeDiff}ms")
        }
