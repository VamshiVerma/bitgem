...
class ChatViewModel(
    ...
    private val meshDelegateHandler = MeshDelegateHandler(
        ...
        onTriggerAiResponse = { aiPrompt ->
            // Trigger AI response when someone else sends a message and we're in AI mode
            commandProcessor.processCommand(aiPrompt, meshService, meshService.myPeerID) { messageContent, mentions, channel ->
    ...
    val aiModeEnabled: LiveData<Boolean> = state.aiModeEnabled
    val swarmRole: LiveData<String> = state.swarmRole
    val swarmMemberRoles: LiveData<Map<String, String>> = state.swarmMemberRoles
        ...
        viewModelScope.launch {
            ...
            if (state.getConnectedPeersValue().isEmpty() && state.getMessagesValue().isEmpty()) {
                val welcomeMessage = BitchatMessage(
                    sender = "system",
                    content = "💎 Welcome to BitGem! Get people around you to download BitGem for secure mesh networking, AI-powered emergency coordination, and private communication!",
                    timestamp = Date(),
                    isRelay = false
                )
    ...
    /**
     * Update an existing message content - for streaming AI responses
     */
    fun updateMessage(messageId: String, newContent: String) {
        messageManager.updateMessage(messageId, newContent)
    }
    
    /**
     * Find a message by ID - for streaming AI responses
     */
    fun findMessage(messageId: String): BitchatMessage? {
        return messageManager.findMessage(messageId)
    }
    ...
    fun toggleAiMode() {
        state.setAiModeEnabled(!state.getAiModeEnabledValue())
    }
    
    /**
     * Add a local message without broadcasting to mesh network (for AI streaming)
     */
    fun addLocalMessage(message: BitchatMessage) {
        messageManager.addMessage(message)
    }
    ...
    // MARK: - Public State Accessors (for SwarmScreen emergency functionality)
    
    fun getConnectedPeers(): List<String> = state.getConnectedPeersValue()
    
    fun getCurrentUserNickname(): String? = state.getNicknameValue()
    
    fun getCurrentPeerID(): String? = "user_peer_id"
    
    // Public accessors for CommandProcessor creation
    fun getChatState(): ChatState = state
    fun getChatMessageManager(): MessageManager = messageManager
    fun getChatChannelManager(): ChannelManager = channelManager
    fun getChatPrivateChatManager(): PrivateChatManager = privateChatManager
}
