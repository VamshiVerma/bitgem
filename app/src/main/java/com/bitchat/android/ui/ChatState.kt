...
class ChatState {
    ...
    // AI mode state
    private val _aiModeEnabled = MutableLiveData<Boolean>(false)
    val aiModeEnabled: LiveData<Boolean> = _aiModeEnabled
    
    // Track messages that AI is already processing to prevent race conditions
    private val _aiProcessingMessages = MutableLiveData<Set<String>>(emptySet())
    val aiProcessingMessages: LiveData<Set<String>> = _aiProcessingMessages
    
    // Swarm role for emergency coordination
    private val _swarmRole = MutableLiveData<String>("")
    val swarmRole: LiveData<String> = _swarmRole
    
    // Swarm member roles (tracks other users' roles)
    private val _swarmMemberRoles = MutableLiveData<Map<String, String>>(emptyMap())
    val swarmMemberRoles: LiveData<Map<String, String>> = _swarmMemberRoles
    ...
    fun getAiModeEnabledValue() = _aiModeEnabled.value ?: false
    fun getAiProcessingMessagesValue() = _aiProcessingMessages.value ?: emptySet()
    fun getSwarmRoleValue() = _swarmRole.value ?: ""
    fun getSwarmMemberRolesValue() = _swarmMemberRoles.value ?: emptyMap()
    ...
    fun addAiProcessingMessage(messageId: String) {
        val current = _aiProcessingMessages.value ?: emptySet()
        _aiProcessingMessages.value = current + messageId
    }
    
    fun removeAiProcessingMessage(messageId: String) {
        val current = _aiProcessingMessages.value ?: emptySet()
        _aiProcessingMessages.value = current - messageId
    }
    
    fun setSwarmRole(role: String) {
        _swarmRole.value = role
    }
    
    fun setSwarmMemberRole(peerID: String, role: String) {
        val current = _swarmMemberRoles.value ?: emptyMap()
        _swarmMemberRoles.value = current + (peerID to role)
    }
    
    fun removeSwarmMemberRole(peerID: String) {
        val current = _swarmMemberRoles.value ?: emptyMap()
        _swarmMemberRoles.value = current - peerID
    }
