...
class MessageManager(private val state: ChatState) {
    ...
    fun updateMessage(messageId: String, newContent: String) {
        ...
        if (messageIndex >= 0) {
            currentMessages[messageIndex] = currentMessages[messageIndex].copy(content = newContent)
            state.setMessages(currentMessages)
        }
        ...
        updatedChannelMessages.forEach { (channel, messages) ->
            ...
            if (channelMessageIndex >= 0) {
                channelMessagesList[channelMessageIndex] = channelMessagesList[channelMessageIndex].copy(content = newContent)
        ...
        var privateUpdated = false
        updatedPrivateChats.forEach { (peerID, messages) ->
            val privateChatMessages = messages.toMutableList()
            val privateMessageIndex = privateChatMessages.indexOfFirst { it.id == messageId }
            if (privateMessageIndex >= 0) {
                privateChatMessages[privateMessageIndex] = privateChatMessages[privateMessageIndex].copy(content = newContent)
                updatedPrivateChats[peerID] = privateChatMessages
                privateUpdated = true
            }
        }
        if (privateUpdated) {
    ...
    /**
     * Find a message by ID across all message lists
     */
    fun findMessage(messageId: String): BitchatMessage? {
        // Check main messages
        state.getMessagesValue().find { it.id == messageId }?.let { return it }
        
        // Check channel messages
        state.getChannelMessagesValue().values.forEach { messages ->
            messages.find { it.id == messageId }?.let { return it }
        }
        
        // Check private chats
        state.getPrivateChatsValue().values.forEach { messages ->
            messages.find { it.id == messageId }?.let { return it }
        }
